...
double NVEensembleMC::getEnergyLRC() { return energyLRC; }
...
void NVEensembleMC::reSetEnergy(double eng) { potEnergy = eng; }
...
void NVEensembleMC::setEnergy() { theEnergy->setEnergy(numAtom, boxLen, &potEnergy); }
...
void NVEensembleMC::lrc() { theEnergy->lrc(numComp, comp, boxVol, &energyLRC); }
...
NVEensembleMC::NVEensembleMC(AtomMC** atoms, int numComp, int numAtom, double totEfixed,
                             double density, double* molFract, int* comp)
    : EnsembleMC(atoms, numComp, numAtom, totEfixed, density, molFract, comp) {
    setVolume();
    setLength();
    setComp();
    initialCoord();
