...
void NVEensemble::setKineticE()
{
  ...
  for(i = 0; i < numAtom; i++)
  {
    ...
    sum += atoms[i]->getMass() * (velocity[0] * velocity[0] +   
...
void NVEensemble::setKineticE(double kinE)
{
   kineticE = kinE;
}
...
{
  theForce->setForce(numAtom, boxLen, &potEnergy, &virial);
}
...
{
  theForce->lrc(numComp, comp, boxVol, &energyLRC, &virialLRC);
}
...
double temperature, double density, double *molFract, int *comp)
