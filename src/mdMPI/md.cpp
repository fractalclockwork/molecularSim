...
void MolecularDynamics::readInNVE(int interPotential)
{
  ...
  if(interPotential == 1) //Lennard-Jones
  {
   ...
   for (i = 0; i < numComp; i++)
   {
     ...
     for (int j = atnum[i]; j < atnum[i+1]; j++)
     {
       ...
       newAtom = new LJatom(i, mass[i], epsilon, sigma,
		     rCut, dimensions, (derivatives - 2), numAtom);
  ...
  ensemble = new NVEensemble(atoms, numComp, numAtom, temperature,
...
double MolecularDynamics::gettStep()
{
  return tStep;
}
