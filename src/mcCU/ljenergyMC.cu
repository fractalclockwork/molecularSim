...
double LJenergy::getTrialPotE(int num, double length, int index, Atom** atom) {
    ...
    energyLJKernel<<<numBlocks, blockSize>>>(num, index, kind_dev, length, rCut_dev, sigma_dev,
                                             epsilon_dev, ri_dev, r_dev, energy_dev);
...
void LJenergy::setEnergy(int num, int numComp, double length, double* potEnergy) {
    int* kind_dev;               // atom type;
    ...
    setLJEnergyKernel<<<numBlocks, blockSize>>>(num, kind_dev, length, rCut_dev, sigma_dev,
                                                epsilon_dev, r_dev, energy_dev);
